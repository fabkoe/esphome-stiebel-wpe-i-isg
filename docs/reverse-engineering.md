...
- [x] **Steigung Heizkurve auslesen** – Index `0x4F2B` bestätigt (0,40→0,45→0,40 verifiziert). Lese-Header `C1 01` (= `generate_read_id(0x601)`) am 29.07. **gerätebestätigt** (war zuvor nur abgeleitet).
  ...
  - [ ] **Komfort-/Eco-Temperatur schreiben (0x4EB8/0x4EB9, auch auf 0x601).** Schreibformat jetzt aus der Heizkurve ableitbar: `C0 01 FA 4E B8/B9 <hi> <lo>` (°C×10). Noch am Gerät zu verifizieren; Display-Sniff der WPM-seitigen Änderung (Screenshots vom 30.07. zeigen Komfort 20,2→20,0 / Eco 16,3→16,0) liegt vor, Log-Mitschnitt der Write-Frames aber noch offen.
